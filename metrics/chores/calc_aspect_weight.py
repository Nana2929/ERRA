...
from utils import load_json, load_jsonl, load_pickle, save_json, save_pickle
...
if __name__ == "__main__":
    ...
    CKPT_PREFIX = f"dset_ver=2_ptr=False_arreg=1.0"
    ROOT = Path(__file__).parent.parent.parent
    if args.auto_arg_by_dataset == "yelp23":
        dargs = edict(
            data_path=f"{ROOT}/nete_format_data/yelp23/reviews.pickle",
            # index_dir=f"{ROOT}/nete_format_data/yelp23/1",
            aspect_path=f"{ROOT}/nete_format_data/yelp23/aspect_category_index.csv",
            # test_filepath=f"{ROOT}/aspect_retriever/checkpoints/{CKPT_PREFIX}/1/yelp23/run_2/explanation_mistral.jsonl",
            prob_path=f"{ROOT}/nete_format_data/yelp23/train_aspect_prob.pickle",
        )
    elif args.auto_arg_by_dataset == "gest":
        dargs = edict(
            data_path=f"{ROOT}/nete_format_data/gest/reviews.pickle",
            # index_dir=f"{ROOT}/nete_format_data/gest/1",
            aspect_path=f"{ROOT}/nete_format_data/gest/aspect_category_index.csv",
            # test_filepath=f"{ROOT}/aspect_retriever/checkpoints/{CKPT_PREFIX}/1/gest/run_3/explanation_mistral.json",
            prob_path=f"{ROOT}/nete_format_data/gest/train_aspect_prob.pickle",
        )

    elif args.auto_arg_by_dataset == "yelp":
        dargs = edict(
            data_path=f"{ROOT}/nete_format_data/yelp/reviews.pickle",
            # index_dir=f"{ROOT}/nete_format_data/yelp/1",
            aspect_path=f"{ROOT}/nete_format_data/yelp/aspect_category_index.csv",
            # test_filepath=f"{ROOT}/aspect_retriever/checkpoints/{CKPT_PREFIX}/1/yelp/run_1/explanation_mistral.json",
            prob_path=f"{ROOT}/nete_format_data/yelp/train_aspect_prob.pickle",
        )
    else:
